...
protected:
  ...
  SEValueInfo RiRegValues[CELL_TIEPOINT_COUNT];														//measured RiReg values (ident=114)
	...
	int ClearValues(int Tiepoint, AdvBaseTypes Type);												//clear values of one type
	//add value	
	int AddValue(int Tiepoint, AdvBaseTypes Type, int Index, SEValueInfo Value);				
...
public:
	...
	int GetLastValue(int Tiepoint, AdvBaseTypes Type, int Index, SEValueInfo * Value );
...
private:
	...
	//-- private memberfunctions	
  int GenerateTestSequence( TestSequenceParam Parameter );										//generate sequence		
